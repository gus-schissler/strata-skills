...
description: Gather the previous day's Slack messages, Gmail, and calendar events for a configured set of sources and post them as one dated markdown document to the connected Strata project over MCP. Built to run as an unattended nightly cloud routine. Project-agnostic: posts to whichever Strata connector is attached and reads its source config from the routine prompt. Trigger when a routine asks to gather yesterday's Slack / email / calendar activity into Strata, or to run the daily Strata gather.
---
